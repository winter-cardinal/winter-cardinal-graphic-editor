...
const SAMPLE_DEFAULT_DIR = path.join( SAMPLE_DIR, "default" );
...
const SAMPLE_JS_WCGE_DIR = path.join( SAMPLE_JS_DIR, "wcardinal-geditor/latest" );

const DOCS_SAMPLE_DIR = "./docs/sample";
const DOCS_SAMPLE_DEFAULT_DIR = path.join( DOCS_SAMPLE_DIR, "default" );
...
const DOCS_SAMPLE_JS_WCGE_DIR = path.join( DOCS_SAMPLE_JS_DIR, "wcardinal-geditor", WCGE_VERSION );
...
const toVersionedSample = ( contents ) => {
	return contents
		.replace( /\/wcardinal-geditor\/latest\//g, `/wcardinal-geditor/${WCGE_VERSION}/` )
...
const makeDocsSample = async ( file ) => {
	...
	const def = toVersionedSample( contents );
	await writeFile( path.join( DOCS_SAMPLE_DEFAULT_DIR, file.dir ), file.name, file.ext, def );
...
const makeIndexMarkdown = async ( dir0, files ) => {
...
const buildIndex = async () => {
	...
	const files = await eachFiles( SAMPLE_DEFAULT_DIR, loadFile, isSampleHTMLFile, true );
	await makeIndexHTML( SAMPLE_DEFAULT_DIR, files );
	logger.end();
};
...
const buildDocs = async () => {
	...
	logger.log( `WinterCardinal Graphic Editor: ${WCGE_VERSION}` );
	logger.end();
	...
	const files = await eachFiles( SAMPLE_DEFAULT_DIR, makeDocsSample, isSampleHTMLFile, true );
	logger.end();
	...
	await eachFiles( SAMPLE_JS_WCGE_DIR, ( file ) => {
		return copyFile( DOCS_SAMPLE_JS_WCGE_DIR, file );
	}, isJSForBrowser );
	logger.end();
	...
	// Create index html
	logger.begin( "Generating index page" );
	await makeIndexMarkdown( DOCS_SAMPLE_DEFAULT_DIR, files );
	logger.end();
};
