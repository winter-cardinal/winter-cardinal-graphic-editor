(function (common) {
	...
	// サブ図面のデータ（グラフィックピースと呼称、JSON形式）の入出力を担当するグラフィックコントローラ
	common.graphicPieceController = {
		search: function( word, categoryId ) {
			// サブ図面データ（JSON）のリストを検索するときに呼び出されます
			// wordには検索文字列が渡されます
			// categoryIdには図面のカテゴリID、またはnull / undefinedが指定されます
			// categoryがnull / undefinedの場合は、全ての図面を検索対象としてください
	...
	// サブ図面にサブ図面を埋め込む際のコントローラを指定します
	// ここでは自分自身を指定しています
	common.graphicPieceController.piece = common.graphicPieceController;
})(window.common ||= {});
